...
test_that(desc = "test nested cv, bayesian, regression - glmnet", code = {
...
test_that(desc = "test nested cv, grid - lightgbm", code = {
...
param_list_xgboost <- expand.grid(
  subsample = seq(0.4, .9, .2),
  colsample_bytree = seq(0.4, .9, .2),
  min_child_weight = seq(1L, 5L, 4L),
  learning_rate = seq(0.1, 0.2, 0.1),
  max_depth = seq(1L, 5L, 4L)
)
...
test_that(desc = "test nested cv, bayesian, reg:squarederror - xgboost", code = {
  testthat::skip_if_not_installed("rBayesianOptimization")
...
test_that(desc = "test nested cv, grid - xgboost", code = {
  xgboost_optimizer <- mlexperiments::MLNestedCV$new(
    learner = mllrnrs::LearnerXgboost$new(
      metric_optimization_higher_better = FALSE
    ),
